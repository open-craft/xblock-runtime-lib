...
With code from:
    ...
    https://github.com/openedx/edx-platform/blob/25b275b/openedx/core/djangoapps/xblock/runtime/blockstore_field_data.py
"""
...
class BlockstoreFieldData(FieldData):
    ...
    """
    def __init__(self, get_olx_hash_for_usage_id):
        """
        ...
        # Requires a method that can fetch the OLX for a given usage key
    ...
    def _get_active_block(self, block):
        ...
        if key not in self.active_blocks:
            self.active_blocks[key] = ActiveBlock(
                olx_hash=self.get_olx_hash_for_usage_id(block.scope_ids.usage_id),
                changed_fields={},
            )
    ...
    def has_cached_definition(self, definition_key):
        ...
        """
        olx_hash = self.get_olx_hash_for_usage_id(definition_key)
